...
export function SwipeCardDeck({ items, posters, onSwipe, onViewAsList, onLike, onAskForHelp }: Props) {
    ...
    if (likedItems.length === 0) {
      return (
        <div className="flex flex-col items-center gap-3 py-8 text-center animate-fade-in">
          ...
          <p className="text-[14px] font-semibold text-foreground">Pasaste todos — ¿buscamos algo diferente?</p>
        </div>
      );
    }
    return (
      <div className="flex flex-col items-center gap-3 py-8 text-center animate-fade-in">
        ...
        <p className="text-[16px] font-bold tracking-tight text-foreground">
          Guardaste {likedItems.length} {likedItems.length === 1 ? "título" : "títulos"}
        </p>
        <p className="text-[12px] text-muted-foreground/60">¿Te ayudo a elegir cuál ver hoy?</p>
        <button
          onClick={() => onAskForHelp?.(likedItems)}
          className="mt-1 rounded-full bg-foreground px-5 py-2 text-[13px] font-semibold text-background transition-opacity hover:opacity-80"
        >
          Sí, ayudame
        </button>
      </div>
  ...
  if (swipingOut === "right") {
    topStyle = {
      transform: "translateX(115%) translateY(-15%) rotate(20deg) scale(0.85)",
